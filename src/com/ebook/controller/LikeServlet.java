...
public class LikeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (CustomSession.sessions.get(sessid) != null) {
			...
			if(InventoryDAO.isFound(CustomSession.sessions.get(sessid).getUsername(), getString.getTarget()) == 1)//if book is found
			{
				LikesDAO.LikeBook(CustomSession.sessions.get(sessid).getUsername(), getString.getTarget());
				json = new Gson().toJson("true");
			} 
			else json = new Gson().toJson("false");//book not found, do nothing
